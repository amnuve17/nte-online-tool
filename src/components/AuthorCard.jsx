...
function YoutubeIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="1.6">
      ...
      <path d="M10 8v8l6.5-4-6.5-4Z" fill="currentColor" stroke="none" />
    </svg>
  );
}
...
export default function AuthorCard({ name, role, image, socials = [] }) {
  return (
    <div className="flex flex-col items-center gap-3 text-center">
      <img src={image} alt={name} className="w-40 shrink-0" />

      <div>
        <p className="font-display text-lg uppercase tracking-wide">{name}</p>
        {role && (
          <p className="font-brand-serif mt-0.5 text-xs italic text-brand-rose/90">
            {role}
          </p>
        )}
      </div>
